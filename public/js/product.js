document.addEventListener("DOMContentLoaded", () => {
  ...
  if (addToCartBtn) {
    addToCartBtn.addEventListener("click", () => {
      ...
      const color = colorInput ? colorInput.value : "Не обрано";
      const image =
        document.querySelector(".basket img")?.getAttribute("src") || "";
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  stars.forEach((star) => {
    star.addEventListener("mouseover", () => {
      ...
      stars.forEach(
        (s) => (s.textContent = Number(s.dataset.value) <= hoverVal ? "★" : "☆")
      );
    });
    ...
    try {
      ...
      if (resp.ok) {
        ...
        slide.innerHTML = `
          ...
          <div class="testimonial-stars">${"★".repeat(data.rating)}${"☆".repeat(
          5 - data.rating
        )}</div>
        `;
